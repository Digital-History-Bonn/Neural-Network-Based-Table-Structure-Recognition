...
if __name__ == "__main__":
    ...
    if not args.BonnData and not args.GloSAT:
        raise ValueError(
            "Please enter a valid dataset must be '--BonnData' or '--GloSAT'!"
        )

    if args.BonnData:
        split_dataset(
            f"{Path(__file__).parent.absolute()}/../../data/BonnData/" f"preprocessed/",
            (0.8, 0.1, 0.1),
        )

    if args.GloSAT:
        split_dataset(
            f"{Path(__file__).parent.absolute()}/../../data/GloSAT/" f"preprocessed/",
            (0.8, 0.1, 0.1),
        )
