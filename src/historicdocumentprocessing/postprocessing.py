...
from src.historicdocumentprocessing.kosmos_eval import extractboxes, reversetablerelativebboxes_outer, calcstats_IoDT, \
    calcmetric, calcstats_IoU, calcstats_overlap, calcmetric_overlap, get_dataframe
from src.historicdocumentprocessing.util.glosat_paper_postprocessing_method import reconstruct_table
...
def postprocess(fullimagepredbox:torch.Tensor, imname:str, saveloc:str, includeoutlier:bool=False):
    #for epsprefactor in [tuple([3.0,1.5]), tuple([4.0,1.5])]:
        ...
        for i, t in enumerate(clusteredtables):
            ...
            rows, colls = reconstruct_table(t.tolist(), tablecoord, eps=4)
