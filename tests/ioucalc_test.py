...
from src.historicdocumentprocessing.kosmos_eval import calcmetric, calcstats_IoU, get_dataframe, \
...
def test_ioumetrics():
    ...
    tpg = torch.sum(prediousp.expand(len(iou_thresholds), n_pred) >= threshold_tensor[:, None], dim=1)
    fpg = len(ioumat) - tpg
    fng = torch.sum(
        targetiousp.expand(len(iou_thresholds), n_target) < threshold_tensor[:, None], dim=1)
    #
...
def test_iousum():
    ...
    """
    df = pandas.read_csv(f"{Path(__file__).parent.absolute()}/../results/fasterrcnn/testevalfinal1/fullimg/Tablesinthewild/testseveralcalls_5_without_valid_split_Tablesinthewild_fullimage_e50_es.pt/iou_0.5_0.9/fullimageiou.csv")
    df1 = get_dataframe(fnsum= torch.Tensor([df['fn@0.5'].sum()]),fpsum= torch.Tensor([df['fp@0.5'].sum()]), tpsum= torch.Tensor([df['tp@0.5'].sum()]), iou_thresholds=[0.5])
    df2 = pandas.read_csv(
...
def test_inputdata():
    full = torch.load(f"{Path(__file__).parent.absolute()}/../data/Tablesinthewild/preprocessed/curved/table_spider_00909/table_spider_00909.pt")
    fromtablerelative = reversetablerelativebboxes_outer(f"{Path(__file__).parent.absolute()}/../data/Tablesinthewild/preprocessed/curved/table_spider_00909")
    ground = processdata_wildtable_inner(f"{Path(__file__).parent.absolute()}/../data/Tablesinthewild/rawdata/test/test-xml-revise/test-xml-revise/table_spider_00909.xml")
    assert torch.equal(full, fromtablerelative), ("table relative and non table relative bounding boxes should be the same when calculating back to whole image")
    assert torch.equal(ground, full), ("processed ground truth bounding boxes should be identical to unprocessed ground truth bounding boxes")
    i=0
    bboxes = []
    with open(f"{Path(__file__).parent.absolute()}/../data/Tablesinthewild/rawdata/test/test-xml-revise/test-xml-revise/table_spider_00909.xml") as d:
        ...
        for box in xml.find_all("bndbox"):
            ...
            if new[0] < new[2] and new[1] < new[3]:
                i+=1
                bboxes.append(new)
    assert torch.equal(ground,torch.vstack(bboxes)), ("unprocessed and processed ground truth should be identical")
    fulltest = torch.load(f"{Path(__file__).parent.absolute()}/../data/Tablesinthewild/test/table_spider_00909/table_spider_00909.pt")
    assert torch.equal(full, fulltest), ("the ground truth bounding boxes used for full testing and testing by category should be identical")
