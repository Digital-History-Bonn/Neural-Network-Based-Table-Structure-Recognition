...
def inferencetest(
    imgpath: str,
    modelpath: str = "microsoft/table-transformer-structure-recognition-v1.1-all",
    num: int = 0,
):
    device = (
    ...
    encoding = move_data_to_device(
        dataset.ImageProcessor(example_image, return_tensors="pt"), device
    )

    with torch.no_grad():
        out = model(**encoding)
    width, height = example_image.size
    pred = dataset.ImageProcessor.post_process_object_detection(
    ...
    img.save(
        f"{Path(__file__).parent.absolute()}/../images/test/testtabletransformerinference_{num}.jpg"
    )
...
def inferencetest2(imgpath: str, modelpath: str = None, num: int = 0):
    ...
    dataloader = DataLoader(
        testdataset, batch_size=1, collate_fn=testdataset.collate_fn
    )
    ...
    pixelvalues = dataset.ImageProcessor(
        example_image, return_tensors="pt", do_pad=False
    )["pixel_values"]
    encoding1 = dataset.ImageProcessor.pad(pixelvalues, return_tensors="pt")
    encoding2 = move_data_to_device(
        dataset.ImageProcessor(image, return_tensors="pt"), device
    )
    with torch.no_grad():
    ...
    pred1 = dataset.ImageProcessor.post_process_object_detection(
        out1, target_sizes=[(height, width)]
    )[0]
    print("boxshape", pred1["boxes"].shape)
    pred1labels = [
        str(model.config.id2label[label]) for label in pred1["labels"].tolist()
    ]
    print(len(pred1labels))
    print([pred1labels == "table row"])
    print(model.config.id2label)
    cells1 = getcells(
        rows=torch.squeeze(pred1["boxes"][pred1["labels"] == 2]),
        cols=torch.squeeze(pred1["boxes"][pred1["labels"] == 1]),
    )
    result = draw_bounding_boxes(
        image=pil_to_tensor(image).to(torch.uint8), boxes=cells1
    )
    img = Image.fromarray(result.permute(1, 2, 0).numpy())
    img.save(
        f"{Path(__file__).parent.absolute()}/../images/test/testtabletransformercellstest_{num}_1.jpg"
    )
    boxes1 = {
    ...
    img = Image.fromarray(result.permute(1, 2, 0).numpy())
    img.save(
        f"{Path(__file__).parent.absolute()}/../images/test/testtabletransformerinference_{num}_1.jpg"
    )
